...
var toByteArray = function toByteArray(str) {
  var byteArray = [];
...
.then(function () {
  ...
  var test = ['from time import sleep', 'def Test():', '    sleep(1)', '', 'Test()', 'class Test2:', '    def __init__(self):', '        sleep(1)', '    def sleepy(self):', '        sleep(1)', '', 'a = Test2()', 'a.sleepy()', '', 'usleep = lambda x: sleep(x/1000000.0)', 'usleep(100) #sleep during 100μs', ''].join('\r\n');

  return vm.exec(test);
}).then(function () {
  var test = ['from time import sleep', '', 'def sleep_decorator(function):', '', '    def wrapper(*args, **kwargs):', '        sleep(2)', '        return function(*args, **kwargs)', '    return wrapper', '', '@sleep_decorator', 'def print_number(num):', '    return num', '', 'print print_number(222)', '', 'for x in range(1,6):', '    print print_number(x)', ''].join('\r\n');
  ...
  var test = ['import js', 'from time import time', '', 'print time()', "get_promise = js.eval('gp')", 'promise = get_promise()', 'res = js.await(promise)', 'print time()', 'print res', "assert res == 'test'"].join('\r\n');
  ...
  var test = ['import js', 'from time import time', '', 'print time()', "get_promise = js.eval('gp')", 'promise = get_promise()', 'res = js.await(promise)', 'print time()'].join('\r\n');
...
.then(function () {
  ...
  var test = ['a = raw_input()', 'print a', "assert a == 'test'"].join('\r\n');
  return vm.exec(test);
})
