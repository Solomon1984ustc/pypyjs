...
(function () {
    ...
    this._ready = new Promise(function (resolve, reject) {
      ...
      var stdout = function stdout(x) {
        ...
        if (c === "\n" || stdout_buffer.length >= 128) {
          _this.stdout(stdout_buffer.join(""));
          stdout_buffer = [];
        }
      };
      ...
      pypyjs._vmBuilderPromise.then(function (vmBuilder) {
        var args = [Module, dependenciesFulfilled, typeof require === 'undefined' ? undefined : require, typeof module === 'undefined' ? undefined : module, typeof __filename === 'undefined' ? undefined : __filename, typeof __dirname === 'undefined' ? undefined : __dirname];
  ...
  pypyjs.prototype.addModuleFromFile = function addModule(name, file) {
    var _this = this;
    return this.fetch(file).then(function (data) {
      return _this.addModule(name, data.responseText);
    });
  }
  ...
  pypyjs.prototype.fetch = function fetch(relpath, responseType) {
    ...
    if (typeof XMLHttpRequest !== "undefined") {
      return new Promise(function (resolve, reject) {
        ...
        var rootURL = _this2.rootURL || pypyjs.rootURL;
  ...
  pypyjs.prototype.exec = function exec(code) {
    ...
    return this._ready.then(function () {
      ...
      if (_this3.autoLoadModules) {
        p = p.then(function () {
          return _this3.findImportedNames(code);
        }).then(function (imports) {
          return _this3.loadModuleData.apply(_this3, imports);
        });
      }
  ...
  pypyjs.prototype.eval = function evaluate(expr) {
    ...
    return this._ready.then(function () {
      ...
      return _this4._execute_source(code);
    }).then(function () {
      return _this4.get("r", true);
    }, function (err) {
  ...
  pypyjs.prototype.repl = function repl(prmpt) {
    ...
    return this._ready.then(function () {
      return _this8.loadModuleData("code");
    }).then(function () {
      return _this8._execute_source("import code");
    }).then(function () {
      return _this8._execute_source("c = code.InteractiveConsole(top_level_scope)");
    }).then(function () {
  ...
  pypyjs.prototype._repl_loop = function _repl_loop(prmpt, ps1) {
    ...
    return Promise.resolve().then(function () {
      ...
      input.split("\n").forEach(function (line) {
        // Find any "import" statements in the code,
        // and ensure the modules are ready for loading.
        if (_this9.autoLoadModules) {
          p = p.then(function () {
            return _this9.findImportedNames(line);
          }).then(function (imports) {
            return _this9.loadModuleData.apply(_this9, imports);
          });
        }
        var code = 'r = c.push(\'' + _escape(line) + '\')';
        p = p.then(function () {
          return _this9._execute_source(code);
        });
      });
      return p;
    }).then(function () {
      return _this9.get("r", true);
    }).then(function (r) {
      // If r == 1, we're in a multi-line definition.
      // Adjust the prompt accordingly.
      if (r) {
        return _this9._repl_loop(prmpt, "... ");
      } else {
        return _this9._repl_loop(prmpt, ">>> ");
      }
    });
  ...
  pypyjs.prototype.loadModuleData = function loadModuleData() /* names */{
    ...
    return this._ready.then(function () {
      ...
      NEXTNAME: for (var i = 0; i < modules.length; i++) {
        ...
        while (true) {
          if (_this10._allModules[name]) {
            break;
          }
          name = name.substr(0, name.lastIndexOf("."));
          if (!name) continue NEXTNAME;
        }
        _this10._findModuleDeps(name, toLoad);
      }
      ...
      for (var name in toLoad) {
        p = p.then(function () {
          return _this10._loadModuleData(name);
        });
      }
      return p;
    });
  ...
  pypyjs.prototype._loadModuleData = function _loadModuleData(name) {
    ...
    var p = this.fetch("modules/" + modfile).then(function (xhr) {
      ...
      _this11._writeModuleFile(name, contents);
      delete _this11._pendingModules[name];
    });
